...
function updateMiniCart(cart, totalItems) {
    ...
    const miniCartFooterSpan = document.querySelector(
        ...
        if (!cart || cart.length === 0) {
            ...
            cart.forEach((item) => {
                ...
                <div class="mini-cart__item">
                    <img src="${
                        item.image || "https://placehold.co/80x80?text=No+Img"
                    }" alt="${item.name}">
                    <div class="mini-cart__info">
                        <a class="mini-cart__name" href="/products/${
                            item.slug
                        }">${item.name}</a>
...
function updateCartPageView(data) {
    ...
    document.getElementById("summary-subtotal").textContent = formatCurrency(
        data.subtotal
    );
    document.getElementById("summary-shipping").textContent = formatCurrency(
        data.shippingFee
    );
    document.getElementById("summary-tax").textContent = formatCurrency(
        data.tax
    );
    ...
    if (data.discountApplied > 0 && data.appliedDiscountCode) {
        document.getElementById(
            "summary-discount"
        ).textContent = `-${formatCurrency(data.discountApplied)}`;
        document.getElementById("discount-code-text").textContent =
            data.appliedDiscountCode;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    cartBody.addEventListener("click", async (e) => {
        ...
        const data = await sendCartRequest(`/api/cart/update/${sku}`, "PATCH", {
        ...
        if (data) {
            ...
            if (newQuantity <= 0) {
                ...
                if (updatedItem) {
                    ...
                    row.querySelector(".cart-total").textContent =
                        formatCurrency(
            ...
            if (errorData) {
                const currentItem = errorData.cart.find(
                    (item) => item.sku === sku
                );
                if (currentItem) {
