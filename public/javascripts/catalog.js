...
window.quickAddToCart = async function(event, btn, productId, sku) {
    ...
    if (event && typeof event.preventDefault === 'function') {
        ...
        event.stopPropagation(); // ngăn click lan ra thẻ cha (tránh mở detail)
    }

    // Nếu btn bị null, thử tìm bằng chính element event.target
    ...
    try {
        ...
        if (response.ok) {
            // Dispatch sự kiện để cập nhật Mini-Cart trên Header
            ...
            // Hiển thị lỗi (có thể nâng cấp thành toast)
            const liveToast = document.getElementById('liveToast');
            const toastMsg = document.getElementById('liveToastMsg');
            if (liveToast && toastMsg && window.bootstrap) {
