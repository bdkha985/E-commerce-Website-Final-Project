...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function createReviewHTML(r) {
        const name =
            r.userId && r.userId.fullName
                ? r.userId.fullName
                : r.fullName || "Người dùng";
        ...
        if (r.sentiment === "Positive")
            sentimentBadge = `<span class="sentiment-badge sentiment-positive">😊 Hài lòng</span>`;
        else if (r.sentiment === "Negative")
            sentimentBadge = `<span class="sentiment-badge sentiment-negative">😞 Thất vọng</span>`;
        else if (r.sentiment === "Neutral")
            sentimentBadge = `<span class="sentiment-badge sentiment-neutral">😐 Trung tính</span>`;
    ...
    function prependReview(review) {
        if (
            review._id &&
            document.querySelector(`.review-item[data-id="${review._id}"]`)
        ) {
            return;
        }

        if (
            reviewListEl.innerHTML.includes("Chưa có đánh giá") ||
            reviewListEl.innerHTML.includes("Đang tải")
        ) {
    ...
    async function fetchReviews(page = 1) {
        if (reviewListEl)
            reviewListEl.innerHTML = '<p class="text-muted">Đang tải...</p>';
        try {
            const sort = sortSelect ? sortSelect.value : "newest";
            const res = await fetch(
                `/api/reviews/${productId}?page=${page}&sort=${sort}`
            );
            const data = await res.json();

            if (data.ok) {
                if (data.reviews.length > 0) {
                    reviewListEl.innerHTML = data.reviews
                ...
                if (totalCountEl)
                    totalCountEl.textContent = data.pagination.totalReviews;
            }
        } catch (e) {
            console.error(e);
        }
