...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const colorChips = colorGroup
        ? Array.from(colorGroup.querySelectorAll(".variant-chip"))
        : [];
    const sizeChips = sizeGroup
        ? Array.from(sizeGroup.querySelectorAll(".variant-chip"))
        : [];
    ...
    let selectedColor =
        colorChips.length === 1 ? colorChips[0].dataset.value : null;
    let selectedSize =
        sizeChips.length === 1 ? sizeChips[0].dataset.value : null;
    ...
    function updateAvailability() {
        ...
        if (colorGroup && sizeGroup) {
            ...
            sizeChips.forEach((c) =>
                c.classList.toggle(
                    "disabled",
                    !availableSizes.has(c.dataset.value)
                )
            );
            colorChips.forEach((c) =>
                c.classList.toggle(
                    "disabled",
                    !availableColors.has(c.dataset.value)
                )
            );
        } else if (colorGroup) {
    ...
    function findMatchingVariant() {
        ...
        const allOptionsSelected =
            (!colorGroup || selectedColor) && (!sizeGroup || selectedSize);
        if (!allOptionsSelected) {
            selectedVariant = null;
            if (btnAddToCart) {
                btnAddToCart.disabled = true;
                btnAddToCart.innerHTML =
                    '<i class="fa-solid fa-cart-plus"></i> Chọn sản phẩm';
            }
            return;
        }
        ...
        if (selectedVariant) {
            if (priceEl)
                priceEl.textContent =
                    (selectedVariant.price || 0).toLocaleString("vi-VN") + "đ";
            ...
            if (
                ...
                const variantImgIndex = allImages.indexOf(
        ...
        try {
            ...
            if (liveToast && toastMsg && window.bootstrap) {
                ...
                toastMsg.textContent =
                    data.message || "Thêm vào giỏ thành công!";
    ...
    colorChips.forEach((chip) =>
        chip.addEventListener("click", () => selectVariant(chip))
    );
    sizeChips.forEach((chip) =>
        chip.addEventListener("click", () => selectVariant(chip))
    );
