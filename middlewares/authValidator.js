...
const signupRules = [
    ...
    body("email")
        ...
        .normalizeEmail(),
    body("address.street").trim().notEmpty().withMessage("Địa chỉ đường là bắt buộc"),
    body("address.ward").trim().notEmpty().withMessage("Phường/Xã là bắt buộc"),
    body("address.city").trim().notEmpty().withMessage("Tỉnh/Thành phố là bắt buộc"),
];
...
const resetPasswordRules = [
    body("email").trim().isEmail().withMessage("Email không hợp lệ"),
    body("resetToken").notEmpty().withMessage("Thiếu mã token đặt lại mật khẩu"),
    body("newPassword")
...
const updateProfileRules = [
    body("fullName")
        .optional()
        .trim()
        .isLength({ min: 2 }).withMessage("Tên quá ngắn (tối thiểu 2 ký tự)"),
    body("phone")
        .optional()
        .trim()
...
const addAddressRules = [
    body("label").trim().notEmpty().withMessage("Tên gợi nhớ (Nhà riêng/Cty) là bắt buộc"),
...
const handleApiValidation = (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
        ...
        return res.status(400).json({
            ...
            errors: errors.array() // Gửi kèm danh sách chi tiết nếu cần debug
        });
    }
    next();
};

// === MIDDLEWARE XỬ LÝ LỖI CHUNG (Cho Form Submit thường - Ít dùng trong dự án này) ===
