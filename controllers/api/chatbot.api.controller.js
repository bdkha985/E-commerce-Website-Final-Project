...
const handleQuery = async (req, res) => {
    try {
        ...
        // 2. Chạy logic AI (AI service sẽ tự xử lý function call)
        ...
        // (Chúng ta chỉ lưu 10 tin nhắn cuối để tránh session quá tải)
        req.session.chatHistory.push({ role: "user", parts: [{ text: message }] });
        req.session.chatHistory.push({ role: "model", parts: [{ text: aiReply }] });
