...
const placeOrder = async (req, res) => {
    ...
    try {
        const { fullName, email, phone, street, ward, city, notes, paymentMethod} = req.body;
        const useLoyalty = (req.body.useLoyalty === true || req.body.useLoyalty === 'true' || req.body.useLoyalty === 'on');
        ...
        const { order, tempPassword } = await orderService.createPendingOrder(req, orderData);

        // 2. Xóa giỏ hàng & session discount
        await cartService.clearCart(req);
        req.session.appliedDiscountCode = null;

        // 3. (MỚI) Lưu MK tạm vào session flash (nếu có)
        // Nó sẽ tồn tại cho đến khi được đọc ở trang kết quả
        if (tempPassword) {
            req.flash('tempPassword', tempPassword);
            req.flash('tempEmail', order.email); // Gửi kèm email
        }
        ...
        res.status(400).json({ ok: false, message: err.message || "Không thể đặt hàng" });
    }
};
