...
// (Hàm getCheckoutPage giữ nguyên)
    ...
    try {
        ...
        if (vnp_Params['vnp_ResponseCode'] === '00') {
            if (order.status !== 'Pending') {
                req.flash('success', `Đơn hàng ${orderCode} đã được xác nhận trước đó.`);
                return res.redirect(`/order/result/${orderCode}`);
            }
            await orderService.completeOrderProcessing(order._id);
            req.flash('success', `Thanh toán thành công cho đơn hàng ${orderCode}!`);
...
const getOrderResultPage = async (req, res, next) => {
    try {
        ...
        // === NÂNG CẤP: LẤY MẬT KHẨU TẠM TỪ SESSION FLASH ===
        // req.flash() sẽ đọc và xóa message ngay lập tức
        ...
        res.render('layouts/main', {
            ...
            order: order,
            tempPassword: tempPassword, // <-- Truyền MK tạm
            tempEmail: tempEmail       // <-- Truyền Email
        });
    } catch (err) {
        next(err);
    }
