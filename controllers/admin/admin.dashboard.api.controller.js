...
const getDateFormat = (groupBy) => {
    switch (groupBy) {
        case 'week': return '%Y-%U'; // Tuần
        case 'month': return '%Y-%m'; // Tháng
        case 'quarter': return { $concat: [ { $toString: { $year: "$createdAt" } }, "-Q", { $toString: { $ceil: { $divide: [{ $month: "$createdAt" }, 3] } }} ] };
        case 'year': return '%Y'; // Năm
        default: return '%Y-%m-%d'; // Ngày
    }
};
...
const getChartData = async (req, res) => {
    try {
        ...
        const defaultStart = moment().startOf('year'); // Mặc định lấy từ đầu năm
        ...
        if (!groupBy) {
            ...
            if (diffDays > 365) groupBy = 'year';       // > 1 năm -> Xem theo Năm
            else if (diffDays > 120) groupBy = 'quarter';// > 4 tháng -> Xem theo Quý
            else if (diffDays > 60) groupBy = 'month';  // > 2 tháng -> Xem theo Tháng
            else if (diffDays > 31) groupBy = 'week';   // > 1 tháng -> Xem theo Tuần
            else groupBy = 'day';                       // Còn lại -> Xem theo Ngày
        }
        ...
        const [orderStats, userStats, topProducts, categoryStats, reviewStats] = await Promise.all([
            ...
            Order.aggregate([
                { $match: { createdAt: { $gte: startDate.toDate(), $lte: endDate.toDate() } } },
                {
                    $facet: {
                        ...
                        timeSeries: [
                            { $match: { paymentStatus: 'Paid' } }, // Chỉ tính đơn đã thanh toán cho Doanh thu/Lợi nhuận
                            {
                                $group: {
                                    _id: { $dateToString: { format: dateFormat, date: "$createdAt" } },
                                    revenue: { $sum: "$total" },
                                    orders: { $sum: 1 },
                                    productsSold: { $sum: { $size: "$items" } } // Đếm số dòng sản phẩm (ước lượng)
                                }
                            },
                            { $sort: { _id: 1 } }
                        ],
                        ...
                        summary: [
                            {
                                $group: {
                                    ...
                                    countCOD: { $sum: { $cond: [{ $eq: ["$paymentMethod", "COD"] }, 1, 0] } },
                                    countVNPAY: { $sum: { $cond: [{ $eq: ["$paymentMethod", "VNPAY"] }, 1, 0] } }
                                }
                            }
                        ]
                    }
            ...
            User.aggregate([
                { $match: { createdAt: { $gte: startDate.toDate(), $lte: endDate.toDate() }, roles: 'customer' } },
                {
                    $group: {
                        _id: { $dateToString: { format: dateFormat, date: "$createdAt" } },
                        count: { $sum: 1 }
            ...
            Order.aggregate([
                { $match: { createdAt: { $gte: startDate.toDate(), $lte: endDate.toDate() }, paymentStatus: 'Paid' } },
                { $unwind: "$items" },
                {
                    $group: {
                        _id: "$items.name",
            ...
            Order.aggregate([
                ...
                {
                    $lookup: { from: "categories", localField: "prod.categoryIds", foreignField: "_id", as: "cat" }
                },
                { $unwind: "$cat" },
                {
                    $group: {
        ...
        const orderMap = orderStats[0].timeSeries.reduce((acc, i) => ({...acc, [i._id]: i}), {});
        const userMap = userStats.reduce((acc, i) => ({...acc, [i._id]: i}), {});

        const chartData = {
            labels: labels,
            revenue: labels.map(l => orderMap[l]?.revenue || 0),
            profit: labels.map(l => (orderMap[l]?.revenue || 0) * 0.3), // Giả lập Lợi nhuận 30%
            orders: labels.map(l => orderMap[l]?.orders || 0),
            newUsers: labels.map(l => userMap[l]?.count || 0)
        };

        // Map Summary Data
        const summary = orderStats[0].summary[0] || { totalOrders: 0, totalRevenue: 0, countCOD: 0, countVNPAY: 0 };
