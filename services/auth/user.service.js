...
async function createUserAndSendPassword({ email, fullName, address }) {
    ...
    try {
        ...
        console.log(
            `[App] Đã đẩy welcome email job cho ${user.email} vào hàng đợi.`
        );
    } catch (err) {
    ...
    try {
        ...
        console.error(
            `[App] Lỗi khi gửi mật khẩu tạm thời cho ${user.email}:`,
            emailError
        );
        throw new Error(
            "Đăng ký thành công nhưng không thể gửi email mật khẩu."
        );
    }
